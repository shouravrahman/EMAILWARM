  ...
  try {
    ...
    const analytics = {
      overview: {
        ...
        openRate: logs.length > 0 ? Math.round((logs.filter(log => log.open_count > 0).length / logs.length) * 100) : 0,
        replyRate: logs.length > 0 ? Math.round((logs.filter(log => log.reply_count > 0).length / logs.length) * 100) : 0,
        bounceRate: logs.length > 0 ? Math.round((logs.filter(log => log.status === 'bounced').length / logs.length) * 100) : 0
      },
...
function generateStatusBreakdown(logs: any[]) {
  ...
  return Object.entries(statusCounts).map(([status, count]) => ({
    status,
    count,
    percentage: logs.length > 0 ? Math.round((count as number / logs.length) * 100) : 0
  }));
}
...
function generateEngagementMetrics(logs: any[]) {
  ...
  return {
    averageOpensPerEmail: logs.length > 0 ? Math.round((totalOpens / logs.length) * 100) / 100 : 0,
    averageRepliesPerEmail: logs.length > 0 ? Math.round((totalReplies / logs.length) * 100) / 100 : 0,
    engagementRate: logs.length > 0 ? Math.round(((logs.filter(log => log.open_count > 0 || log.reply_count > 0).length) / logs.length) * 100) : 0,
...
function generateRecipientAnalysis(logs: any[]) {
  ...
  return Array.from(domainMap.values())
    .map(domain => ({
      ...domain,
      openRate: domain.sent > 0 ? Math.round((domain.opened / domain.sent) * 100) : 0,
      replyRate: domain.sent > 0 ? Math.round((domain.replied / domain.sent) * 100) : 0,
      bounceRate: domain.sent > 0 ? Math.round((domain.bounced / domain.sent) * 100) : 0
    }))
