...
export class AdvancedAIService {
  ...
  async generateContextualEmail(request: EmailGenerationRequest): Promise<EmailGenerationResponse> {
    try {
      ...
      const emailResponse = this.parseAdvancedEmailResponse(text, request);
  ...
  async analyzeEmailAdvanced(subject: string, body: string, context?: EmailContext): Promise<EmailAnalysisResponse> {
    try {
      const prompt = this.buildAdvancedAnalysisPrompt(subject, body, context);
      const result = await this.model.generateContent(prompt);
      const response = await result.response;
      const text = response.text();
  ...
  async optimizeEmailSequence(emails: Array<{ subject: string; body: string; timestamp: string }>): Promise<{
  ...
  private buildAdvancedEmailPrompt(request: EmailGenerationRequest): string {
    ...
    if (context.previousEmails && context.previousEmails.length > 0) {
      conversationHistory = `
        ...
        ${email.direction === 'sent' ? 'You sent' : 'They sent'} (${email.timestamp}):
        Subject: ${email.subject}
        Body: ${email.body}
        `).join('\n')}
      `;
    ...
    return `
      ...
      Campaign Goal: ${context.campaignGoal || 'Build sender reputation through natural conversations'}
      ...
      ${constraints?.avoidWords?.length ? `- Avoid these words: ${constraints.avoidWords.join(', ')}` : ''}
      ${constraints?.mustIncludeWords?.length ? `- Must include: ${constraints.mustIncludeWords.join(', ')}` : ''}
  ...
  private buildAdvancedAnalysisPrompt(subject: string, body: string, context?: EmailContext): string {
    return `
      ...
      - Recipient: ${context.recipientName || 'Unknown'} (${context.recipientEmail || 'Unknown'})
  ...
  private parseAdvancedEmailResponse(text: string, request: EmailGenerationRequest): EmailGenerationResponse {
    try {
      ...
      if (jsonMatch) {
        ...
        return {
          ...
          reasoning: parsed.reasoning || 'Generated using advanced AI'
        };
      }
    } catch (error) {
    ...
    const subject = this.extractSubject(text) || 'Professional Communication';
    ...
    try {
      ...
      if (jsonMatch) {
        ...
        return {
          ...
          deliverabilityScore: Math.max(0, Math.min(100, parsed.deliverabilityScore || 80)),
          engagementScore: Math.max(0, Math.min(100, parsed.engagementScore || 60)),
          reputationImpact: Math.max(0, Math.min(100, parsed.reputationImpact || 70)),
  ...
  private extractBody(text: string): string | null {
    const bodyMatch = text.match(/(?:body|content|message):\s*["']?([\s\S]+?)["']?(?:\n\n|\n"|\n}|$)/i);
  ...
  private performFallbackAnalysis(content: string): EmailAnalysisResponse {
    const spamWords = [
      ...
      'cash', 'money', 'earn', 'income', 'investment', 'loan', 'credit',
      'viagra', 'casino', 'lottery', 'prize', 'offer expires'
    ];
    ...
    if (!contentLower.includes('dear') && !contentLower.includes('hi ') && !contentLower.includes('hello')) {
      engagementScore -= 15;
    ...
    if (content.length < 50) {
      engagementScore -= 20;
      suggestions.push('Email content is too short for meaningful engagement');
    ...
    return {
      ...
      deliverabilityScore,
      engagementScore,
      reputationImpact: Math.round((deliverabilityScore + engagementScore) / 2),
      suggestions,
      issues,
      optimizations
    };
  ...
  private async logEmailGeneration(request: EmailGenerationRequest, response: EmailGenerationResponse): Promise<void> {
    try {
      await supabase
  ...
  async getConversationContext(emailId: string, recipientEmail: string): Promise<EmailContext['previousEmails']> {
    try {
